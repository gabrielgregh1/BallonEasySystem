...
public class produto_vendidoDAO {
    ...
    public List<Produto_vendido> read(String mes){
        ...
        try {
            stmt = con.prepareStatement("select resumida, dataVenda,  sum(qtdProdutoVendido), (sum(qtdProdutoVendido)*preco) from  produto_vendido, produto, venda  " +
                                       "where fk_produto = codigoProduto and fk_venda = codigoVenda and dataVenda " +
                                        "like ?" +
"                                       group by codigoProduto  order by sum(qtdProdutoVendido) desc;");
    ...
    public List<Produto_vendido> readDespesas(String mes){
        Produto_vendido produto_vendidoObjeto;
        Venda vendaObjeto ;
        Produto produtoObjeto;
        List<Produto_vendido>lista = new ArrayList();
        ...
        try {
            stmt = con.prepareStatement("select resumida, dataVenda,  sum(qtdProdutoVendido), (sum(qtdProdutoVendido)*preco), custoUnidade from  produto_vendido, produto, venda  " +
                                       "where fk_produto = codigoProduto and fk_venda = codigoVenda and dataVenda " +
                                        "like ?" +
"                                       group by codigoProduto  order by sum(qtdProdutoVendido) desc;");
            stmt.setString(1, "%-"+mes+"-%");
            ...
            while(rs.next()){
                ...
                double qtdProdutoVendido = rs.getDouble(3);
                double preco = rs.getDouble(4);
                double custoUnidade = rs.getDouble(5);
   
                produtoObjeto = new Produto(resumida, custoUnidade);
                vendaObjeto = new Venda(dataVenda, preco);
                produto_vendidoObjeto = new Produto_vendido(produtoObjeto, vendaObjeto, qtdProdutoVendido);
                lista.add(produto_vendidoObjeto);
            }
